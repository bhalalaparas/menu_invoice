...
from bedrock_usage_log import logger as bedrock_logger, log_bedrock_call
import os
import io
import re
import json
...
def _bedrock_log_source(
    source: str,
    page_number: Optional[int],
    total_pages: Optional[int],
) -> str:
    """Tag multi-page invoice runs so usage logs show one row per page, not ambiguous duplicates."""
    if (
        source == "invoice"
        ...
        return f"invoice_page_{page_number}_of_{total_pages}"
    return source
...
def call_bedrock_model(
    ...
    One invoke per PDF page; pass page_number/total_pages for correct usage-log labeling.
    """
    content = [{"type": "text", "text": prompt}]
    log_source = _bedrock_log_source(source, page_number, total_pages)
    ...
    try:
        bedrock_logger.debug("invoke_model source=%s model=%s", log_source, MODEL_ID)
        response = BRT.invoke_model(
            modelId='arn:aws:bedrock:us-east-1:078805859846:inference-profile/us.anthropic.claude-opus-4-7',
        ...
        log_bedrock_call(
            source=log_source,
            model_id='arn:aws:bedrock:us-east-1:078805859846:inference-profile/us.anthropic.claude-opus-4-7',
            prompt=prompt,
            output=out_text,
        ...
        bedrock_logger.exception("Bedrock invoke failed source=%s: %s", log_source, e)
        traceback.print_exc()
        return ""
